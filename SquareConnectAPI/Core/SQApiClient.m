...
#import "SQJSONRequestSerializer.h"
#import "SQQueryParamCollection.h"
#import "SQDefaultConfiguration.h"

NSString *const SQResponseObjectErrorKey = @"SQResponseObject";

static NSString * const kSQContentDispositionKey = @"Content-Disposition";

static NSDictionary * SQ__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * SQ__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = SQ__headerFieldsForResponse(response);
    if(!headers[kSQContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kSQContentDispositionKey];
...
@interface SQApiClient ()

@property (nonatomic, strong, readwrite) id<SQConfiguration> configuration;
...
- (instancetype)init {
    return [self initWithConfiguration:[SQDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithBaseURL:(NSURL *)url {
    return [self initWithBaseURL:url configuration:[SQDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithConfiguration:(id<SQConfiguration>)configuration {
    return [self initWithBaseURL:[NSURL URLWithString:configuration.host] configuration:configuration];
}

- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<SQConfiguration>)configuration {
    self = [super initWithBaseURL:url];
    if (self) {
        _configuration = configuration;
        _timeoutInterval = 60;
        _responseDeserializer = [[SQResponseDeserializer alloc] init];
        _sanitizer = [[SQSanitizer alloc] init];

        _downloadTaskResponseTypes = @[@"NSURL*", @"NSURL"];

        AFHTTPRequestSerializer* afhttpRequestSerializer = [AFHTTPRequestSerializer serializer];
        SQJSONRequestSerializer * swgjsonRequestSerializer = [SQJSONRequestSerializer serializer];
        _requestSerializerForContentType = @{kSQApplicationJSONType : swgjsonRequestSerializer,
...
- (NSURLSessionDataTask*) taskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask *task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        SQDebugLogResponse(response, responseObject,request,error);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[SQResponseObjectErrorKey] = responseObject;
        }
...
- (NSURLSessionDataTask*) downloadTaskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask* task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString *filename = SQ__fileNameForResponse(response);
...
- (NSURLSessionTask*) requestWithPath: (NSString*) path
    ...
    __weak id<SQSanitizer> sanitizer = self.sanitizer;
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = SQPercentEscapedStringFromString(safeString);
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path queryParams:(NSDictionary*) queryParams {
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = SQPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, SQPercentEscapedStringFromString(queryParam)]];
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = SQPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = SQPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = SQPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (void) updateHeaderParams:(NSDictionary * *)headers queryParams:(NSDictionary * *)querys WithAuthSettings:(NSArray *)authSettings {
    ...
    id<SQConfiguration> config = self.configuration;
